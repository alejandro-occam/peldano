...
const actions = {
    ...
    async getInfoOrder({ state }, id){
        try {
            ...
            //Rellenar objetos para el store y mostrar la información de las propuestas
            createObjectsStoreOrders({ state }, response);
...
function createObjectsStoreOrders({ state }, response){
    var array_services = response.data.array_services;
    state.orders.proposal_obj.products.articles = [];
    state.orders.proposal_obj.products.dates_prices_aux = [];
    ...
    state.orders.proposal_obj.products = array_products;

    //Consultamos los totales
    var total_amount_global = 0;
    var total_individual_pvp = 0;
    var total_global = 0;
    state.orders.proposal_obj.products.map(function(product, key) {
        product.articles.map(function(article, key) {
            total_individual_pvp += article.article_obj.pvp;
            article.dates_prices.map(function(date_price, key) {
                date_price.arr_pvp_date.map(function(pvp_date, key) {
    ...
    state.orders.proposal_obj.total_global_normal = total_global;
    ...
    array_bills.map(function(bill, key) {
        ...
        state.orders.bill_obj.array_bills.push(bill_obj);
        state.orders.bill_obj.articles = bill.array_articles;
    });

    state.orders.bill_obj.total_bill = total_bill;*/
    //Guardamos con un nuevo formato para las facturas los articulos
    var array_articles = [];
    state.orders.proposal_obj.products.map(function(products, key) {
        products.articles.map(function(article_obj, key) {
            article_obj.dates_prices.map(function(dates_prices_obj, key) {
                dates_prices_obj.arr_pvp_date.map(function(arr_pvp_date_obj, key) {
                    arr_pvp_date_obj.arr_pvp.map(function(arr_pvp_obj, key) {
    ...
    //Guardamos datos
    state.orders.proposal_bd_obj = proposal_submission_settings;
    state.orders.proposal_obj.array_dates = array_dates_prices;
    state.orders.status_view = 3;
    state.errors.type_error = 'get_info_proposal';
    state.errors.code = response.data.code;
    state.orders.is_change_get_info = 1;
    state.orders.id_company = response.data.proposal.id_company;
}
