...
class GetContactsCompanies extends Command
{
    ...
    public function handle()
    {
        ...
        while($stop_companies == 0){
            $array_companies_hubspot_obj = json_decode($requ_curls->getCurl($url)['response'], true);
            foreach($array_companies_hubspot_obj["results"] as $company_hubspot){
                ...
                while($stop_companies_association_contacts == 0){
                    $array_companies_association_contacts_hubspot_obj = json_decode($requ_curls->getCurl($url_contacts_association)['response'], true);
                    foreach($array_companies_association_contacts_hubspot_obj["results"] as $company_association_contacts){
                        ...
                        if(!$contact){
                            ...
                            $contact_hubspot_obj = json_decode($requ_curls->getCurl($url_contact)['response'], true);
                            Contact::create([
