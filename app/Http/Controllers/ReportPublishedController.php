...
class ReportPublishedController extends Controller
{
    ...
    function reportsList(Request $request){
        ...
        $array_bills_orders_dig = Service::select('services.*', 'departments.nomenclature as department_nomenclature', 
                                            ...
                                            ->leftJoin('services_bills_orders', 'services_bills_orders.id_service', 'services.id')
            ...
            if($key == 0){
                ...
                foreach($array_dates_old as $key_date => $date){
                    if(strtotime($date['last_date_custom2']) >= strtotime($bill_order_dig->date) && strtotime($date['first_date_custom2']) <= strtotime($bill_order_dig->date)){
                        $custom_obj_old['amounts'][] = round($bill_order_dig->pvp, 2);
                    }else{
                ...
                foreach($array_dates as $key_date => $date){
                    if(strtotime($date['last_date_custom2']) >= strtotime($bill_order_dig->date) && strtotime($date['first_date_custom2']) <= strtotime($bill_order_dig->date)){
                        $custom_obj_new['amounts'][] = round($bill_order_dig->pvp, 2);
                    }else{
                ...
                if($exist){
                    ...
                    foreach($array_dates_old as $key_date => $date){
                        if(strtotime($date['last_date_custom2']) >= strtotime($bill_order_dig->date) && strtotime($date['first_date_custom2']) <= strtotime($bill_order_dig->date)){
                            $array_bills_orders_custom[$position]['old']['amounts'][$key_date] += round($bill_order_dig->pvp, 2);
                        }
                    }
                    foreach($array_dates as $key_date => $date){
                        if(strtotime($date['last_date_custom2']) >= strtotime($bill_order_dig->date) && strtotime($date['first_date_custom2']) <= strtotime($bill_order_dig->date)){
                            $array_bills_orders_custom[$position]['new']['amounts'][$key_date] += round($bill_order_dig->pvp, 2);
                        }
                    }
                }
            }
